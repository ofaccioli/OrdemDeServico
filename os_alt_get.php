...
<?php
	...
	if($ini == ""){
		...
		if($_SESSION["admin"] == true){
			...
			if($cod == "todos"){
				...
				$sql = "select r.Cod_os, r.Data, c.Nome_colaborador, d.Nome_setor,r.Prioridade as Prioridade,r.Prioridade_tec as Prioridade2, r.Finalizado as Finalizado, r.Titulo_OS from os r inner join Colaborador c on r.Cod_colaborador = c.Cod_colaborador inner join setor d on r.Cod_setor = d.Cod_setor where r.Data between '".$ini."' and '".$fim."' order by r.Cod_os desc limit 50";
				
			}else{
				$sql = "select r.Cod_os, r.Data, c.Nome_colaborador, d.Nome_setor,r.Prioridade as Prioridade,r.Prioridade_tec as Prioridade2, r.Finalizado as Finalizado, r.Titulo_OS from os r inner join Colaborador c on r.Cod_colaborador = c.Cod_colaborador inner join setor d on r.Cod_setor = d.Cod_setor where c.Cod_colaborador = ".$cod." and r.Data between '".$ini."' and '".$fim."' order by r.Cod_os desc limit 50";
			}
	...
	echo "<form method='post' action='os_alt_rec.php'>";
